...
A library's rendered result may be a document node or an ordinary
expression value depending on the imported program.

vals exposes its top-level binding values.
Partial imports
...
and the result may immediately be field-accessed:

$partial({"items": $ctx.items}).rendered

A partial may be curried across multiple parameters.
Stabilized import semantics

Imports should be statically identifiable.

The import target should therefore be represented in the core AST as a
statically specified import name rather than an arbitrary runtime expression.

This allows implementations to determine dependencies without executing
arbitrary expressions.

Partial imports preserve the same static dependency while deferring parameter
completion.
18. Concat
